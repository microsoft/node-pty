...
function defer(terminal, deferredFn) {
    ...
    // Halt execution if the terminal is dead. This usually occurs
    // after a Terminal.prototype.kill is called.
    if(terminal.isDead) {
    ...
    terminal.deferreds.push({
       run: function() {
          ...
          // Do not run any deffereds if terminal is dead (killed).   
          if(terminal.isDead) {
              // TODO: Should we throw instead?
              return;
          }
