...
function Agent(file, args, env, cwd, cols, rows, debug) {
  ...
  console.log({file: file, cmdline: cmdline, /*env: env, */cwd: cwd, cols:cols, rows:rows, debug:debug});
  var p = pty.startProcess(file, cmdline, env, cwd, cols, rows, debug);
  console.log('p');
  console.log(p);
  this.dataPipeIn = pty.getDataPipeIn(handle);
  this.dataPipeOut = pty.getDataPipeOut(handle);
console.log('dataPipeIn: ' + this.dataPipeIn);
console.log('dataPipeOut: ' + this.dataPipeOut);
